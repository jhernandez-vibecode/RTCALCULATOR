# Cotizador RT Construcción — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Formulario HTML embebible en Google Sites que captura 4 datos del cliente y envía automáticamente la cotización del Seguro RT Construcción al correo del cliente junto con una notificación al agente.

**Architecture:** Un archivo HTML single-file (cotizador-construccion.html) hace POST a un Google Apps Script (Code.gs) que valida, calcula la prima, guarda en Sheets y envía 2 correos. El precio no aparece en la página — solo llega al correo del cliente.

**Tech Stack:** Google Apps Script (V8), GmailApp, CacheService, SpreadsheetApp, HTML/CSS/JS vanilla (sin dependencias externas)
...
| Archivo | Rol |
|---------|-----|
| `appsscript.json` | Manifest: runtime V8, timezone, acceso anónimo |
| `Code.gs` | doPost + validación + cálculo + Sheets + emails |
| `cotizador-construccion.html` | Formulario embebible, estilos, animación checkmark |

---

## Task 1: Manifest + constantes base

**Files:**
- Create: `appsscript.json`
- Create: `Code.gs`

- [ ] **Step 1: Crear appsscript.json**
...
- [ ] **Step 2: Crear Code.gs con constantes del agente**
...
```bash
git add appsscript.json Code.gs
git commit -m "feat: setup inicial appsscript + constantes agente"
```

---

## Task 2: Crear Google Sheet + función de logging

**Files:**
- Modify: `Code.gs`

- [ ] **Step 1: Crear la hoja en Google Sheets**

Ir a sheets.google.com → crear hoja nueva → nombrarla "Cotizador RT Construcción"
→ Crear pestaña "Cotizaciones RT" → agregar headers en fila 1:

```
A: Fecha | B: Nombre | C: Teléfono | D: Correo | E: Valor Obra | F: Monto Asegurado | G: Prima | H: Estado
```

→ Copiar el ID de la URL (`https://docs.google.com/spreadsheets/d/[ESTE_ID]/edit`) y reemplazar `PENDIENTE_REEMPLAZAR` en `Code.gs`.

- [ ] **Step 2: Agregar función de logging en Code.gs**
...
- [ ] **Step 3: Probar la función desde Apps Script**

En el editor de Apps Script: seleccionar `_registrarCotizacion` en el dropdown → Run con estos datos de prueba:

```javascript
// Ejecutar manualmente para probar
_registrarCotizacion({
  nombre: 'Test Cliente',
  telefono: '88001234',
  correo: 'test@test.com',
  montoObra: 50000000,
  montoAsegurado: 17500000,
  prima: 696500
});
```

Verificar que aparece la fila en el Sheet.

- [ ] **Step 4: Commit**

```bash
git add Code.gs
git commit -m "feat: logging en Google Sheets (Cotizaciones RT)"
```

---

## Task 3: Validación + rate limit + cálculo

**Files:**
- Modify: `Code.gs`

- [ ] **Step 1: Agregar funciones de validación y cálculo**
...
- [ ] **Step 2: Verificar cálculo manualmente en Apps Script**

Ejecutar en consola del editor:

```javascript
// En el editor: Tools > Script editor > console
Logger.log(_calcular(39715083));
// Esperado: { montoAsegurado: 13900279.05, prima: 553231.10... }
Logger.log(_formatCRC(553231));
// Esperado: ₡ 553,231
```
...
```bash
git add Code.gs
git commit -m "feat: validacion + rate limit + calculo prima RT"
```

---

## Task 4: doPost — entrada principal

**Files:**
- Modify: `Code.gs`

- [ ] **Step 1: Implementar doPost completo**
...
- [ ] **Step 2: Agregar doGet para verificar que el deploy funciona**
...
```bash
git add Code.gs
git commit -m "feat: doPost principal con validacion, calculo y dispatch de emails"
```

---

## Task 5: Email al cliente

**Files:**
- Modify: `Code.gs`

- [ ] **Step 1: Implementar _enviarEmailCliente**
...
- [ ] **Step 2: Probar email cliente desde Apps Script**

Crear función temporal de prueba en Code.gs, ejecutarla y revisar el correo recibido:

```javascript
function _testEmailCliente() {
  _enviarEmailCliente({
    nombre:          'Roy González López',
    telefono:        '8800-1234',
    correo:          'jhernandez@segurosdelins.com', // enviar a vos mismo
    montoObra:       39715083,
    montoAsegurado:  13900279,
    prima:           553231
  });
}
```

Verificar en Gmail: formato, valores, links de WhatsApp, footer SDI.

- [ ] **Step 3: Commit**

```bash
git add Code.gs
git commit -m "feat: email HTML al cliente con cotizacion y requisitos RT"
```

---

## Task 6: Notificación al agente

**Files:**
- Modify: `Code.gs`

- [ ] **Step 1: Implementar _enviarNotificacionAgente**
...
- [ ] **Step 2: Probar notificación**

```javascript
function _testNotifAgente() {
  _enviarNotificacionAgente({
    nombre:          'Roy González López',
    telefono:        '8800-1234',
...
Ejecutar y verificar en Gmail: tabla de datos, botón WhatsApp funciona con mensaje pre-cargado.

- [ ] **Step 3: Eliminar funciones _test* antes del deploy final**

Borrar `_testEmailCliente` y `_testNotifAgente` de Code.gs.

- [ ] **Step 4: Commit**

```bash
git add Code.gs
git commit -m "feat: notificacion al agente con link WhatsApp directo al cliente"
```

---

## Task 7: HTML — estructura, estilos e iframe

**Files:**
- Create: `cotizador-construccion.html`

- [ ] **Step 1: Crear el archivo base con estilos**
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .card {
    ...
    width: 100%;
    max-width: 480px;
    box-shadow: 0 16px 40px -12px rgba(11,31,61,.18), 0 4px 12px -4px rgba(11,31,61,.08);
  }

  /* ── Header ── */
  .header {
    background: linear-gradient(135deg, var(--ins-blue) 0%, var(--ins-blue-l) 100%);
    padding: 20px 24px;
    text-align: center;
  }
  .header-badge {
  ...
  .divider {
    ...
    color: var(--gray-400); font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: .5px;
  }
  ...
  .spin-ring {
    width: 22px; height: 22px; border: 2.5px solid rgba(255,255,255,.4);
  ...
  .check-wrap svg circle {
    stroke: rgba(255,255,255,.35); stroke-width: 2; fill: none;
    stroke-dasharray: 56; stroke-dashoffset: 56;
    animation: circle-draw .4s ease forwards;
  }
  .check-wrap svg polyline {
    stroke: #fff; stroke-width: 2.5; fill: none;
    stroke-linecap: round; stroke-linejoin: round;
    stroke-dasharray: 22; stroke-dashoffset: 22;
    animation: check-draw .35s ease .35s forwards;
  }
  ...
  .success-msg {
    ...
    font-weight: 600;
    margin-top: 8px;
    animation: fade-up .4s ease .6s both;
  }
  ...
  .error-msg {
    ...
    padding: 10px 14px; font-size: 12px; color: #B91C1C; text-align: center;
  }
...
<div class="card">
  ...
  <form id="cotizadorForm" class="form-body" novalidate>
    ...
    <div class="field field-monto">
      ...
      <input type="number" id="montoObra" name="montoObra" placeholder="Ej. 50000000" min="1" required>
    ...
    <div class="success-msg" id="successMsg">
      ✅ Cotización enviada · revisá tu correo en instantes
    </div>
  ...
  <div class="form-footer">
    🔒 Tu información es confidencial · Cotización válida 15 días
  </div>
</div>

<script>
// URL del Web App — REEMPLAZAR después del deploy
const WEBAPP_URL = 'PENDIENTE_REEMPLAZAR';
...
form.addEventListener('submit', async (e) => {
  ...
  const body = new URLSearchParams({
    ...
    montoObra: document.getElementById('montoObra').value.trim(),
  ...
  try {
    ...
    if (data.status === 'ok') {
      ...
      btn.disabled = false;
      errMsg.textContent = data.msg || 'Error al enviar. Intentá de nuevo.';
...
function validarForm() {
  let ok = true;
  ['nombre','telefono','correo','montoObra'].forEach(id => {
    const el = document.getElementById(id);
    const val = el.value.trim();
    if (!val || (id === 'correo' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val))) {
...
- [ ] **Step 2: Abrir el archivo en un browser y verificar visualmente**

```
Abrir cotizador-construccion.html en Chrome.
Verificar: header azul, campos bien distribuidos, campo monto destacado, botón visible.
Probar animación: en la consola del browser ejecutar `document.getElementById('btnSubmit').classList.add('success')` para ver el checkmark sin hacer POST real.
```

- [ ] **Step 3: Commit**

```bash
git add cotizador-construccion.html
git commit -m "feat: formulario HTML cotizador RT construccion"
```

---

## Task 8: Deploy + conectar URL + prueba end-to-end

**Files:**
- Modify: `cotizador-construccion.html` (reemplazar WEBAPP_URL)

- [ ] **Step 1: Subir código a Apps Script y hacer deploy**

1. Ir a [script.google.com](https://script.google.com) → crear proyecto nuevo → nombrar "Cotizador RT Construcción"
2. Reemplazar el contenido con `Code.gs` y crear `appsscript.json`
3. Click **Deploy → New deployment**
   - Type: Web app
   - Execute as: Me (juan carlos)
   - Who has access: Anyone
4. Copiar la URL del Web App (formato: `https://script.google.com/macros/s/AKfy.../exec`)

- [ ] **Step 2: Reemplazar WEBAPP_URL en el HTML**

En `cotizador-construccion.html`, línea con `const WEBAPP_URL`:

```javascript
const WEBAPP_URL = 'https://script.google.com/macros/s/[TU_DEPLOYMENT_ID]/exec';
```

- [ ] **Step 3: Verificar doGet funciona**

Abrir la URL del Web App en el browser. Debe mostrar:
```json
{"status":"online","app":"Cotizador RT Construcción"}
```

- [ ] **Step 4: Prueba end-to-end completa**

Abrir `cotizador-construccion.html` en el browser y llenar:
- Nombre: `Test Construcción`
- Teléfono: `8800-1111`
- Correo: `jhernandez@segurosdelins.com`
- Monto: `50000000`

Verificar:
- [ ] Checkmark verde se anima correctamente
- [ ] Correo llega a jhernandez@segurosdelins.com (como cliente y como notificación)
- [ ] Fila aparece en Google Sheets
- [ ] Valores calculados son correctos: monto asegurado ₡17,500,000 · prima ₡696,500

- [ ] **Step 5: Commit final**

```bash
git add cotizador-construccion.html
git commit -m "feat: conectar formulario a Web App — cotizador RT listo para Google Sites"
```

---

## Task 9: Embed en Google Sites

- [ ] **Step 1: Subir cotizador-construccion.html al repo de GitHub**

```bash
git push origin main
```

- [ ] **Step 2: Obtener URL raw del HTML (GitHub Pages o raw)**

Opción A — GitHub Pages: activar en Settings → Pages → Branch: main → carpeta raíz.
La URL quedará: `https://jhernandez-vibecode.github.io/RT-construccion-appscript/cotizador-construccion.html`

Opción B — Netlify Drop: arrastrar el archivo a app.netlify.com/drop → obtener URL pública.

- [ ] **Step 3: Embeber en Google Sites**

1. Abrir la página de Google Sites donde va el cotizador
2. Insertar → Embed → Por URL → pegar la URL del HTML
3. Ajustar el alto del iframe (recomendado: 560px)
4. Publicar el sitio

- [ ] **Step 4: Prueba final desde Google Sites**

Llenar el formulario desde la página publicada y confirmar que todo funciona igual que en local.

---

## Checklist de entrega final

- [ ] `appsscript.json` — manifest correcto (V8, ANYONE_ANONYMOUS, Costa Rica)
- [ ] `Code.gs` — doPost, validación, cálculo, Sheets, 2 emails
- [ ] `cotizador-construccion.html` — formulario, estilos, checkmark, WEBAPP_URL real
- [ ] Email cliente: valor obra + prima + resumen datos + requisitos (4 pasos) + footer SDI
- [ ] Email agente: tabla datos + botón WhatsApp con mensaje pre-cargado
- [ ] Honeypot activo + rate limit 30s verificados
- [ ] Fila en Google Sheets después de cada envío
- [ ] Embed funcionando en Google Sites
